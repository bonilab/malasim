...
protected:
  ...
  std::unique_ptr<SpatialData> spatial_data{nullptr};
  ...
  void SetUp() {
    ...
    spatial_data = std::make_unique<SpatialData>(&Model::get_config()->get_spatial_settings());
  ...
  YAML::Node createBasicNode() {
    ...
    for (int i = 0; i < 8; i++) {
      YAML::Node age_dist;
      for (const auto &val : dist) { age_dist.push_back(val); }
  ...
  void TearDown() {
    ...
    if (Model::get_config() != nullptr) { Model::get_instance()->set_config(nullptr); }
