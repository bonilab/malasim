...
public:
    MOCK_METHOD(double, get_current_parasite_density, (ClonalParasitePopulation* parasite, int duration), (override));
};
...
TEST_F(ClonalParasitePopulationTest, Initialization) {
    ...
    EXPECT_EQ(cpp->last_update_log10_parasite_density(), ClonalParasitePopulation::LOG_ZERO_PARASITE_DENSITY);
...
TEST_F(ClonalParasitePopulationTest, GetLog10InfectiousDensity) {
    ...
    cpp->set_gametocyte_level(0.5);
    EXPECT_EQ(cpp->get_log10_infectious_density(), ClonalParasitePopulation::LOG_ZERO_PARASITE_DENSITY);
...
TEST_F(ClonalParasitePopulationTest, PerformDrugAction) {
    ...
    EXPECT_DOUBLE_EQ(cpp->last_update_log10_parasite_density(), 4.6989700043360187); // 5.0 + log10(0.5)
...
TEST_F(ClonalParasitePopulationTest, GetCurrentParasiteDensityWithUpdateFunction) {
    ...
    // Set up expectations
    EXPECT_CALL(*static_cast<MockSingleHostClonalParasitePopulations*>(parasite_population), latest_update_time())
...
TEST_F(ClonalParasitePopulationTest, PerformDrugActionEdgeCases) {
    ...
    EXPECT_NEAR(cpp->last_update_log10_parasite_density(), 5.0 + log10(1 - percent_parasite_remove), 1e-10);
    ...
    EXPECT_THROW(cpp->perform_drug_action(percent_parasite_remove, log10_parasite_density_cured), std::invalid_argument);
}
...
TEST_F(ClonalParasitePopulationTest, GetCurrentParasiteDensityNullUpdateFunction) {
    ...
    cpp->set_last_update_log10_parasite_density(ClonalParasitePopulation::LOG_ZERO_PARASITE_DENSITY);
    EXPECT_DOUBLE_EQ(cpp->get_current_parasite_density(10), ClonalParasitePopulation::LOG_ZERO_PARASITE_DENSITY);
