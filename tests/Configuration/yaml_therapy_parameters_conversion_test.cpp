...
protected:
    ...
    void SetUp() override {
        ...
        therapy_info_mono_as.set_drug_id({0});
        therapy_info_mono_as.set_dosing_days({3});

        TherapyParameters::TherapyInfo therapy_info_mono_lumefaltrine;
        therapy_info_mono_lumefaltrine.set_drug_id({1});
        therapy_info_mono_lumefaltrine.set_dosing_days({3});

        // Add more therapy information as necessary
        std::vector<TherapyParameters::TherapyInfo> therapy_db = {
            therapy_info_mono_as,
            therapy_info_mono_lumefaltrine,
            // Add more therapy entries as needed for testing...
        };
...
TEST_F(TherapyParametersTest, EncodeTherapyParameters) {
    ...
    EXPECT_EQ(node["therapy_db"][0]["drug_id"].as<std::vector<int>>(), std::vector<int>{0});
...
TEST_F(TherapyParametersTest, DecodeTherapyParameters) {
    ...
    node["therapy_db"][0]["drug_id"] = std::vector<int>{0};
    node["therapy_db"][0]["dosing_days"] = std::vector<int>{3};

    node["therapy_db"][1]["drug_id"] = std::vector<int>{1};
    ...
    EXPECT_EQ(decoded_parameters.get_therapy_db()[0].get_drug_id(), std::vector<int>{0});
    EXPECT_EQ(decoded_parameters.get_therapy_db()[0].get_dosing_days(), std::vector<int>{3});
    EXPECT_EQ(decoded_parameters.get_therapy_db()[1].get_drug_id(), std::vector<int>{1});
    EXPECT_EQ(decoded_parameters.get_therapy_db()[1].get_dosing_days(), std::vector<int>{3});
}
