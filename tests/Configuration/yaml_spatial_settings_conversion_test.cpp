...
protected:
  ...
  void SetUp() override {
    ...
    grid.set_age_distribution_by_location(
...
TEST_F(SpatialSettingsTest, EncodeSpatialSettings) {
  ...
  EXPECT_EQ(node["grid_based"]["population_raster"].as<std::string>(),
            grid.get_population_raster());
  EXPECT_EQ(node["grid_based"]["p_treatment_under_5_raster"].as<std::string>(),
            grid.get_p_treatment_under_5_raster());
  EXPECT_EQ(node["grid_based"]["p_treatment_over_5_raster"].as<std::string>(),
            grid.get_p_treatment_over_5_raster());
  ...
  for (size_t i = 0; i < grid.get_age_distribution_by_location().size(); ++i) {
    CompareVectors(grid.get_age_distribution_by_location()[i],
                   node["grid_based"]["age_distribution_by_location"]
                       .as<std::vector<std::vector<double>>>()[i],
                   21);
  }
}
...
TEST_F(SpatialSettingsTest, DecodeSpatialSettings) {
  ...
  node["grid_based"]["administrative_boundaries"][0]["raster"] =
      "../sample_inputs/kag_district.asc";
  ...
  for (size_t i = 0; i < grid.get_age_distribution_by_location().size(); ++i) {
    CompareVectors(
        grid.get_age_distribution_by_location()[i],
        std::vector<std::vector<double>>{{0.0378, 0.0378, 0.0378, 0.0378, 0.0282, 0.0282, 0.0282,
                                          0.0282, 0.0282, 0.029,  0.029,  0.029,  0.029,  0.029,
                                          0.169,  0.134,  0.106,  0.066,  0.053,  0.035,  0.0}}[i],
        21);
  }
  ...
  for (size_t i = 0; i < location.get_age_distribution_by_location().size(); ++i) {
    CompareVectors(
        location.get_age_distribution_by_location()[i],
        std::vector<std::vector<double>>{{0.0378, 0.0378, 0.0378, 0.0378, 0.0282, 0.0282, 0.0282,
                                          0.0282, 0.0282, 0.029,  0.029,  0.029,  0.029,  0.029,
                                          0.169,  0.134,  0.106,  0.066,  0.053,  0.035,  0.0}}[i],
        21);
