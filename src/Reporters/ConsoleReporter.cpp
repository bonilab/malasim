...
void report_number_by_state(const int &location, PersonIndexByLocationStateAgeClass* pi) {
  ...
  for (int hs = 0; hs < Person::NUMBER_OF_STATE - 1; hs++) {
    ...
    //        }
    double v = Model::get_instance().get_mdc()->popsize_by_location_hoststate()[location][hs] * 100 /
               (double) Model::get_instance().get_mdc()->popsize_by_location()[location];
    //        double v = sum;
...
void ConsoleReporter::after_run() {
  ...
  //total time
  double total_time_in_years = (Model::get_instance().get_scheduler()->current_time()
  ...
  for (int location = 0; location < Model::get_instance().get_config()->get_spatial_settings().get_number_of_locations(); location++) {
    std::cout << Model::get_instance().get_mdc()->EIR_by_location()[location] << "\t";
  }
  std::cout << std::endl;
  ...
  for (int location = 0; location < Model::get_instance().get_config()->get_spatial_settings().get_number_of_locations(); location++) {
    std::cout << Model::get_instance().get_mdc()->total_number_of_bites_by_location()[location] << "\t";
  }
  std::cout << std::endl;

  std::cout << "Number of clinical episodes:" << std::endl;
  for (int location = 0; location < Model::get_instance().get_config()->get_spatial_settings().get_number_of_locations(); location++) {
    std::cout << Model::get_instance().get_mdc()->cumulative_clinical_episodes_by_location()[location] << "\t";
  }
  std::cout << std::endl;

  std::cout << "Percentage of bites on top 20% bitten" << std::endl;
  for (int location = 0; location < Model::get_instance().get_config()->get_spatial_settings().get_number_of_locations(); location++) {
    std::cout << Model::get_instance().get_mdc()->percentage_bites_on_top_20_by_location()[location] * 100 << "%" << "\t";
  }
  std::cout << std::endl;

  std::cout << "NTF by location: " << std::endl;
  for (int location = 0; location < Model::get_instance().get_config()->get_spatial_settings().get_number_of_locations(); location++) {
    double location_NTF = Model::get_instance().get_mdc()->cumulative_NTF_by_location()[location] * 100 /
                          (double) Model::get_instance().get_mdc()->popsize_by_location()[location];
  ...
  for (int location = 0; location < Model::get_instance().get_config()->get_spatial_settings().get_number_of_locations(); location++) {
    std::cout << Model::get_instance().get_mdc()->cumulative_mutants_by_location()[location] << "\t";
  }
  std::cout << std::endl;
  ...
  std::cout << "Strategy UTL: " << Model::get_instance().get_mdc()->current_utl_duration() << std::endl;

  std::cout << "AMU per parasite population: " << Model::get_instance().get_mdc()->AMU_per_parasite_pop() << std::endl;
  std::cout << "AMU per per: " << Model::get_instance().get_mdc()->AMU_per_person() << std::endl;
  std::cout << "EAMU count only clinical caused parasite: "
            << Model::get_instance().get_mdc()->AMU_for_clinical_caused_parasite()
            << std::endl;
...
void ConsoleReporter::monthly_report() {
  ...
  if (Model::get_instance().get_scheduler()->current_time() % Model::get_instance().get_config()->get_model_settings().get_days_between_stdout_output() == 0) {
//        Model::get_instance().get_mdc()->perform_population_statistic();

    std::cout << Model::get_instance().get_scheduler()->current_time() << "\t";
    ...
    for (int location = 0; location < Model::get_instance().get_config()->get_spatial_settings().get_number_of_locations(); location++) {
      ...
      std::cout << Model::get_instance().get_mdc()->blood_slide_prevalence_by_location()[location] * 100 << "\t";
      std::cout << Model::get_instance().get_mdc()->total_immune_by_location()[location] / Model::get_instance().get_population()->size(location)
                << "\t";
      std::cout << Model::get_instance().get_mdc()->current_RITF_by_location()[location] << "-"
                << Model::get_instance().get_mdc()->current_TF_by_location()[location] << "\t";
    }
    std::cout << std::endl;
  }
}