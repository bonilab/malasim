...
void ValidationReporter::initialize(int job_number, const std::string &path) {
    // Define file paths
    std::string monthly_data_path = fmt::format("{}/validation_monthly_data_{}.txt", path, job_number);
    ...
    std::string monthly_mutation_path = fmt::format("{}/validation_monthly_mutation_{}.txt", path, job_number);
    std::string mosquito_res_count_path = fmt::format("{}/validation_mosquito_res_count_{}.txt", path, job_number);
    ...
    monthly_mutation_logger = spdlog::basic_logger_mt("validation_monthly_mutation", monthly_mutation_path);
    mosquito_res_count_logger = spdlog::basic_logger_mt("validation_mosquito_res_count", mosquito_res_count_path);
...
void ValidationReporter::monthly_report() {
    ...
    for (auto loc = 0; loc < Model::get_config()->number_of_locations(); loc++) {
        if ((Model::get_mdc()->popsize_by_location_hoststate()[loc][Person::ASYMPTOMATIC] + Model::get_mdc()->popsize_by_location_hoststate()[loc][Person::CLINICAL]) == 0){
            ...
            ss << Model::get_mdc()->popsize_by_location_hoststate()[loc][Person::ASYMPTOMATIC] /
                  (Model::get_mdc()->popsize_by_location_hoststate()[loc][Person::ASYMPTOMATIC] + Model::get_mdc()->popsize_by_location_hoststate()[loc][Person::CLINICAL])  << sep;
        }
        ss << group_sep;//159
    }
    for (auto loc = 0; loc < Model::get_config()->number_of_locations(); loc++) {
    ...
    for (auto loc = 0; loc < Model::get_config()->number_of_locations(); loc++) {
        for (auto ac = 0; ac < Model::get_config()->number_of_age_classes(); ac++){
            ...
            if (all_infected_pop == 0){
                ...
                ss << std::setprecision(6) << Model::get_mdc()->number_of_clinical_by_location_age_group()[loc][ac] / (double) all_infected_pop<< sep;
            }
        }
        ss << group_sep;//337
    }
    ...
    for (auto loc = 0; loc < Model::get_config()->number_of_locations(); loc++) {
        ...
        if (all_infected_pop == 0){
            ...
            ss << std::setprecision(6) << Model::get_mdc()->monthly_number_of_clinical_episode_by_location()[loc] / (double) all_infected_pop<< sep;
        }
        ss << group_sep;//341
    }
    for (auto loc = 0; loc < Model::get_config()->number_of_locations(); loc++) {
...
//                                              Model::get_population()->get_person_index<PersonIndexByLocationStateAgeClass>());
    ...
    for (auto loc = 0; loc < Model::get_config()->number_of_locations(); loc++) {
        ...
        for (int i = 0; i < Model::get_mdc()->mutation_tracker[loc].size(); i++) {
            ...
            ss << std::get<1>(Model::get_mdc()->mutation_tracker[loc][i]) << sep;
            ss << std::get<2>(Model::get_mdc()->mutation_tracker[loc][i]) << sep;
            ss << std::get<3>(Model::get_mdc()->mutation_tracker[loc][i]) << sep;
            ss << std::get<4>(Model::get_mdc()->mutation_tracker[loc][i]) << sep;
            ss << std::get<5>(Model::get_mdc()->mutation_tracker[loc][i]) << '\n';
        }
    }
    if(sum > 0){
    ...
    for (auto loc = 0; loc < Model::get_config()->number_of_locations(); loc++) {
        ...
        for (int i = 0; i < Model::get_mdc()->mosquito_recombined_resistant_genotype_tracker[loc].size(); i++) {
            ...
            ss << std::get<1>(Model::get_mdc()->mosquito_recombined_resistant_genotype_tracker[loc][i]) << sep;
            ss << std::get<2>(Model::get_mdc()->mosquito_recombined_resistant_genotype_tracker[loc][i]) << sep;
            ss << std::get<3>(Model::get_mdc()->mosquito_recombined_resistant_genotype_tracker[loc][i]) << '\n';
        }
    }
    if(sum > 0){
        ...
        for (auto [g_id, genotype] : *(Model::get_config()->get_genotype_parameters().genotype_db)) {
            if(resistant_drug_pair_id < 3){
                spdlog::debug(fmt::format("resistant_drug_pair_id: {} {}\tR-0: {}\tR-1: {}\tEC50-0: {}\tEC50-1: {}\tminEC50-0: {}\tminEC50-1: {}",
                                       ...
                                       pow(Model::get_config()->get_drug_parameters().drug_db->at(drugs[0])->base_EC50, Model::get_config()->get_drug_parameters().drug_db->at(drugs[0])->n()),
                                       pow(Model::get_config()->get_drug_parameters().drug_db->at(drugs[1])->base_EC50, Model::get_config()->get_drug_parameters().drug_db->at(drugs[1])->n())));
            }
            else{
              spdlog::debug(fmt::format("resistant_drug_pair_id: {} {}\tR-0: {}\tR-1: {}\tR-2: {}\tEC50-0: {}\tEC50-1: {}\tEC50-2: {}\tminEC50-0: {}\tminEC50-1: {}\tminEC50-2: {}",
                                       ...
                                       pow(Model::get_config()->get_drug_parameters().drug_db->at(drugs[0])->base_EC50, Model::get_config()->get_drug_parameters().drug_db->at(drugs[0])->n()),
                                       pow(Model::get_config()->get_drug_parameters().drug_db->at(drugs[1])->base_EC50, Model::get_config()->get_drug_parameters().drug_db->at(drugs[1])->n()),
                                       pow(Model::get_config()->get_drug_parameters().drug_db->at(drugs[1])->base_EC50, Model::get_config()->get_drug_parameters().drug_db->at(drugs[2])->n())));
            }
        }
        spdlog::debug("###############");
    }
