...
SpatialData::SpatialData() { data = new AscFile*[SpatialFileType::Count](); }

SpatialData::~SpatialData() {
  if (data == nullptr) { return; }
  for (auto ndx = 0; ndx != SpatialFileType::Count; ndx++) {
    if (data[ndx] != nullptr) { delete data[ndx]; }
  }
  delete[] data;
}
...
void SpatialData::generate_locations() {
  ...
  if (ndx == SpatialFileType::Count) {
    spdlog::error("No spatial file found to generate locations with!");
    return;
  }
...
void SpatialData::load_raster(SpatialFileType type) {
    ...
    for (auto col = 0; col < values->NCOLS; col++) {
      ...
      if (id > count) {
        throw std::runtime_error(
            fmt::format("Raster misalignment: pixel count exceeds {} expected "
                        "while loading raster type {}",
                        count, static_cast<uint32_t>(type)));
      }
      ...
      switch (type) {
        ...
        case Population:
          ...
          if (values->data[row][col] != ceil(static_cast<double>(values->data[row][col]))) {
            spdlog::warn(
                "Population data lost at row {}, col {}, value {}", row, col,
                values->data[row][col]);
          }
          location_db[id].population_size = static_cast<int>(values->data[row][col]);
          break;
        case PrTreatmentUnder5:
          location_db[id].p_treatment_under_5 = values->data[row][col];
          break;
        case PrTreatmentOver5:
          location_db[id].p_treatment_over_5 = values->data[row][col];
          break;
        default:
          throw std::runtime_error(
              fmt::format("{} called with invalid raster, type id: {}",
                          __FUNCTION__, static_cast<uint32_t>(type)));
      }
      id++;
    }
  }
  ...
  if ((unsigned)(id) != count) {
    throw std::runtime_error(
        fmt::format("Raster misalignment: found {} pixels, expected {} while "
                    "loading raster type {}",
                    id, count, static_cast<uint32_t>(type)));
  }

  // Log the updates
  spdlog::info("Loaded values from raster file, type id: {}", static_cast<uint32_t>(type));
}
...
bool SpatialData::parse(const YAML::Node &node) {
  ...
  // Load the treatment information, note that if rasters were loaded via
  // refresh() this will end up being a NOP
  for (auto loc = 0; loc < number_of_locations; loc++) {
    if (!node[TREATMENT_RATE_UNDER5]) {
      // Set the under 5 value
      auto input_loc = node["p_treatment_under_5_by_location"].size()
                               < number_of_locations
                           ? 0
                           : loc;
      location_db[loc].p_treatment_under_5 = node["p_treatment_under_5_by_location"][input_loc]
              .as<double>();
    }
    if (!node[TREATMENT_RATE_OVER5]) {
      // Set the over 5 value
      auto input_loc = node["p_treatment_over_5_by_location"].size()
                               < number_of_locations
                           ? 0
                           : loc;
      location_db[loc].p_treatment_over_5 = node["p_treatment_over_5_by_location"][input_loc]
              .as<double>();
    }
  }
  ...
  if (!node[BETA_RASTER]) {
    for (auto loc = 0; loc < number_of_locations; loc++) {
      ...
      location_db[loc].beta = node["beta_by_location"][input_loc].as<double>();
    }
  }
...
void SpatialData::populate_dependent_data() {
  ...
  if (districts.front() == 0) {
    if ((districts.size() - 1) != district_count
      ...
      spdlog::error("Highest district ID is inconsistent with a zero-based "
                    "index, expected {}, got {}",districts.size(), districts.back());
    ...
    if (districts.size() != district_count
      ...
      spdlog::error("Highest district ID is inconsistent with a one-based "
                    "index, expected {}, got {}",districts.size(), districts.back());
      throw std::invalid_argument(
          "District raster inconsistently numbered, or contains invalid "
          "data.");
    ...
    spdlog::error("Index of first district must be zero or one, found {}",districts.front());
    throw std::invalid_argument(
  ...
  spdlog::info("District lookup populated with {} districts",district_lookup_.size());
}
...
void SpatialData::refresh() {
  ...
  std::string errors;
  if (dirty && check_catalog(errors)) { throw std::runtime_error(errors); }
