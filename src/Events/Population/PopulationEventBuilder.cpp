...
std::vector<WorldEvent*> PopulationEventBuilder::build_introduce_parasites_periodically_events_v2(const YAML::Node& node,
                                                                                             Config* config) {
  std::vector<WorldEvent*> events;
  for (const auto& event_node : node) {
    ...
    for (auto loc = location_from; loc < location_to; ++loc) {
      for (auto j = 0; j < event_node["parasite_info"].size(); j++) {
        ...
        auto end_time = (date::sys_days { end_date } - date::sys_days { config->get_simulation_timeframe().get_starting_date() }).count();
...
PopulationEventBuilder::build_change_treatment_strategy_event(
  ...
  for (const auto &entry : node) {
    ...
    if (strategy_id >= config->get_strategy_parameters().strategy_db.size()) {
      spdlog::error("Invalid strategy_id! {} supplied, but strategy_db size is {}",
...
std::vector<WorldEvent*> PopulationEventBuilder::build_turn_on_mutation_event(const YAML::Node& node, Config* config) {
  std::vector<WorldEvent*> events;
  for (const auto& event_node : node) {
...
std::vector<WorldEvent*> PopulationEventBuilder::build_turn_off_mutation_event(const YAML::Node& node, Config* config) {
  std::vector<WorldEvent*> events;
  for (const auto& event_node : node) {
    const auto starting_date = event_node["date"].as<date::year_month_day>();
    auto time = (date::sys_days { starting_date } - date::sys_days { config->get_simulation_timeframe().get_starting_date() }).count();
...
std::vector<WorldEvent*> PopulationEventBuilder::build_change_interrupted_feeding_rate_event(const YAML::Node& node,
                                                                                        Config* config) {
  std::vector<WorldEvent*> events;
  for (const auto& event_node : node) {
...
std::vector<WorldEvent*> PopulationEventBuilder::build_change_within_host_induced_free_recombination_events(const YAML::Node node,
                                                                                           Config* config) {
  std::vector<WorldEvent*> events;
  for (const auto& event_node : node) {
    const auto starting_date = event_node["date"].as<date::year_month_day>();
...
std::vector<WorldEvent*> PopulationEventBuilder::build_change_mutation_probability_per_locus_events(const YAML::Node node,
                                                                                       Config* config) {
  std::vector<WorldEvent*> events;
  for (const auto& event_node : node) {
    const auto starting_date = event_node["date"].as<date::year_month_day>();
...
PopulationEventBuilder::build_introduce_lumefantrine_mutant_parasite_events(
  ...
  for (const auto &entry : node) {
    ...
    if (static_cast<std::size_t>(location) < config->number_of_locations()) {
      ...
      auto* event =
          new IntroduceLumefantrineMutantEvent(location, time, fraction,alleles);
      events.push_back(event);
    }
  }
  return events;
...
std::vector<WorldEvent*> PopulationEventBuilder::build_introduce_580Y_mutant_events(const YAML::Node& node, Config* config) {
  std::vector<WorldEvent*> events;
  for (const auto& event_node : node) {
    auto location = event_node["location"].as<int>();
    if (location <config->number_of_locations()) {
...
std::vector<WorldEvent*> PopulationEventBuilder::build_introduce_triple_mutant_to_dpm_parasite_events(const YAML::Node& node,
                                                                                                 Config* config) {
  std::vector<WorldEvent*> events;
  for (const auto& event_node : node) {
    auto location = event_node["location"].as<int>();
...
PopulationEventBuilder::build_rotate_treatment_strategy_event(
  ...
  try {
    ...
    spdlog::error(
        ...
        "{} node: {}",RotateStrategyEvent::EventName,
        error.msg);
    exit(EXIT_FAILURE);
  }
}
...
std::vector<WorldEvent*> PopulationEventBuilder::build_annual_beta_update_event(
  ...
  try {
    ...
    spdlog::debug(
        "Adding {} start: {}, rate: {}",
        event->name(), StringHelpers::date_as_string(start_date), rate);
    std::vector<WorldEvent*> events;
    events.push_back(event);
    return events;
  } catch (YAML::BadConversion &error) {
    spdlog::error(
        "Unrecoverable error parsing YAML value in "
        "{} node: {}",AnnualBetaUpdateEvent::EventName,
        error.msg);
    exit(EXIT_FAILURE);
  }
}
...
std::vector<WorldEvent*> PopulationEventBuilder::build_annual_coverage_update_event(
  ...
  try {
    ...
    spdlog::error(
        "Unrecoverable error parsing YAML value in "
        "{} node: {}",AnnualCoverageUpdateEvent::EventName,
        error.msg);
    exit(EXIT_FAILURE);
  }
}
...
PopulationEventBuilder::build_change_circulation_percent_event(
  ...
  try {
    ...
    for (const auto &entry : node) {
      ...
      if (rate < 0.0) {
        spdlog::error("The daily population circulation percentage must be "
                      "greater than zero for {}",ChangeCirculationPercentEvent::EventName);
        throw std::invalid_argument(
    ...
    spdlog::error(
        "Unrecoverable error parsing YAML value in "
        "{} node: {}",ChangeCirculationPercentEvent::EventName,
        error.msg);
    exit(EXIT_FAILURE);
  }
}
  ...
  try {
    ...
    for (const auto &entry : node) {
      ...
      if (start_date.day() != date::day{1}) {
        spdlog::error(
            "The event must start on the first of the month for {} ",
      ...
      if (genotype_id < 0) {
        spdlog::error(
            "Invalid genotype id supplied for {} genotype id cannot be less than zero",
            ImportationPeriodicallyRandomEvent::EventName);
      ...
      if (log_parasite_density == 0) {
        spdlog::error("Log parasite density cannot be zero for "
      ...
      spdlog::debug(
          "Adding {} start: {}, genotype_id: {}, count: {}, log_parasite_density: {}",
            event->name(), StringHelpers::date_as_string(start_date), genotype_id, count, log_parasite_density);
      events.push_back(event);
    }
    return events;
  } catch (YAML::BadConversion &error) {
    spdlog::error(
        "Unrecoverable error parsing YAML value in "
        "{} node: {}",ImportationPeriodicallyRandomEvent::EventName,
        error.msg);
    exit(EXIT_FAILURE);
  }
}
...
std::vector<WorldEvent*> PopulationEventBuilder::build_update_beta_raster_event(
  ...
  try {
    ...
    for (const auto &entry : node) {
      ...
      spdlog::debug(
          "Adding {} start: {}, filename: {}",
          event->name(), StringHelpers::date_as_string(start_date), filename);
      events.push_back(event);
    }
    return events;
  } catch (YAML::BadConversion &error) {
    spdlog::error(
        "Unrecoverable error parsing YAML value in "
        "{} node: {}",UpdateBetaRasterEvent::EventName,
        error.msg);
    exit(EXIT_FAILURE);
  }
}
...
void PopulationEventBuilder::verify_single_node(const YAML::Node &node,
  ...
  if (node.size() > 1) {
    spdlog::error(
        "More than one sub node found for " + name + " in the configuration file");
  }
}
