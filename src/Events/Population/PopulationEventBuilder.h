...
public:
  ...
  static std::vector<Event*> build_introduce_parasite_events(
      const YAML::Node &node, Config* config);

  static std::vector<Event*> build_introduce_parasites_periodically_events(
      const YAML::Node &node, Config* config);

  static std::vector<Event*> build_introduce_parasites_periodically_events_v2(const YAML::Node& node, Config* config);

  static std::vector<Event*> build_change_treatment_coverage_event(
      const YAML::Node &node, Config* config);
  ...
  static std::vector<Event*> build_introduce_plas2_parasite_events(
  const YAML::Node &node, Config* config);
  ...
  static std::vector<Event*> build_change_within_host_induced_free_recombination_events(const YAML::Node node, Config* pConfig);

  static std::vector<Event*> build_change_mutation_probability_per_locus_events(const YAML::Node node, Config* pConfig);

  static std::vector<Event*> build_change_interrupted_feeding_rate_event(const YAML::Node& node, Config* config);

  static std::vector<Event*> build_introduce_amodiaquine_mutant_parasite_events(const YAML::Node &node, Config* config);
  ...
  static std::vector<Event*> build_annual_coverage_update_event(
      const YAML::Node &node, Config* config);

  static std::vector<Event*> build_annual_beta_update_event(
      const YAML::Node &node, Config* config);

  static std::vector<Event*> build_change_circulation_percent_event(
      const YAML::Node &node, Config* config);

  static std::vector<Event*> build_importation_periodically_random_event(
      const YAML::Node &node, Config* config);

  static std::vector<Event*> build_update_beta_raster_event(
      const YAML::Node &node, Config* config);
