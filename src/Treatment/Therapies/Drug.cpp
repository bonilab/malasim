...
double Drug::get_current_drug_concentration(int current_time) {
  const auto days = current_time - start_time_;
  if (days == 0) { return 0; }
  ...
  if (days <= dosing_days_) {
    if (drug_type()->id() == 0) {
      ...
      // std::cout << "days: " << days << " start: " << starting_value_ << " uniform: " <<
      // starting_value_temp_ << " = " << starting_value_ + starting_value_temp_ << std::endl;
    ...
    const auto temp = NumberHelpers::is_equal(drug_type_->drug_half_life(), 0.0)
                          ? -100
                          : -(days - dosing_days_) * log(2)
                                / drug_type_->drug_half_life();  //-ai*t = - t* ln2 / tstar
    if (exp(temp) <= (10.0 / 100.0)) { return 0; }
