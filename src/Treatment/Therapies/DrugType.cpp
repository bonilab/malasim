...
double DrugType::get_parasite_killing_rate_by_concentration(
    const double &concentration, const double &EC50_power_n) {
  ...
  const auto killing_perday = maximum_parasite_killing_rate_
                              * (con_power_n / (con_power_n + EC50_power_n));
...
int DrugType::get_total_duration_of_drug_activity(
    const int &dosing_days) const {
  // CutOffPercent is 10
...
void DrugType::populate_resistant_aa_locations() {
  ...
  for (const auto &chromosome_info : Model::get_config()
                                         ->get_genotype_parameters()
                                         .get_pf_genotype_info()
                                         .chromosome_infos) {
    for (int gene_id = 0; gene_id < chromosome_info.get_genes().size();
         ++gene_id) {
      ...
      auto aa_pos_in_sequence =
          Model::get_config()
              ->get_genotype_parameters()
              .get_pf_genotype_info()
              .calculate_aa_pos(chromosome_info.get_chromosome_id() - 1,
                                gene_id, 0);

      for (int aa_id = 0; aa_id < gene_info.get_aa_positions().size();
           ++aa_id) {
        for (auto const &multiple_ec50 :
             gene_info.get_aa_positions()[aa_id]
                 .get_multiplicative_effect_on_EC50()) {
          if (multiple_ec50.get_drug_id() == id_) {
            resistant_aa_locations.push_back(
                {chromosome_info.get_chromosome_id() - 1, gene_id, aa_id,
      ...
      if (gene_info.get_max_copies() > 1) {
        for (auto const &cnv_multiple_ec50 :
             gene_info.get_cnv_multiplicative_effect_on_EC50()) {
          if (cnv_multiple_ec50.get_drug_id() == id_) {
            resistant_aa_locations.push_back(
                {chromosome_info.get_chromosome_id() - 1, gene_id,
                 static_cast<int>(gene_info.get_aa_positions().size()),
                 aa_pos_in_sequence, true});
          }
        }
      }
    }
