...
void MFTRebalancingStrategy::update_end_of_time_step() {
  ...
  if (Model::get_instance().get_scheduler()->current_time()==latest_adjust_distribution_time) {
    // actual trigger adjust distribution
    for (auto i = 0; i < distribution.size(); i++) {
      distribution[i] = next_distribution[i];
    }
    next_update_time = Model::get_instance().get_scheduler()->current_time() + update_duration_after_rebalancing;
    std::cout << date::year_month_day{Model::get_instance().get_scheduler()->calendar_date} << ": MFT Rebalancing adjust distribution: "
              << to_string();
    //            std::cout << to_string() << std::endl;
  } else {
    if (Model::get_instance().get_scheduler()->current_time()==next_update_time) {
      double sum = 0;
      for (auto i = 0; i < distribution.size(); i++) {
        ...
        if (Model::get_instance().get_mdc()->current_tf_by_therapy()[therapy_list[i]->get_id()] < 0.05) {
          next_distribution[i] = 1.0/0.05;
        } else {
          next_distribution[i] = 1.0/Model::get_instance().get_mdc()->current_tf_by_therapy()[therapy_list[i]->get_id()];
        }
      ...
      latest_adjust_distribution_time = Model::get_instance().get_scheduler()->current_time() + delay_until_actual_trigger;
