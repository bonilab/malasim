...
void NovelDrugIntroductionStrategy::monthly_update() {
  ...
  if (!is_switched) {
    ...
    if (Model::get_scheduler()->current_time() > 3000 && Model::get_mdc()->current_tf_by_therapy()[current_public_therapy_id] >= tf_threshold) {
      ...
      auto novel_SFT_strategy = Model::get_config()->get_strategy_parameters().strategy_db[newly_introduced_strategy_id];
      ...
      Model::get_config()->get_strategy_parameters().strategy_db.push_back(new_public_stategy);
