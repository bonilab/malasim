...
class DbLoader {
public:
  // Load the genotypes into the database, assume that these are the same for
  // ALL configurations
  static bool load_genotypes(const std::string &configuration) {
    const std::string INSERT_GENOTYPE =
        "INSERT INTO sim.Genotype (id, name) VALUES ({}, '{}');";
    ...
    try {
      ...
      for (auto id = 0ul; id < config->number_of_parasite_types(); id++) {
        ...
        query.append(
            fmt::format(INSERT_GENOTYPE, id, genotype->to_string(config)));
      }
