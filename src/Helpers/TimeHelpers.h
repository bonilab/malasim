 ...
 * TimeHelpers.h
 *
 * Defines and implements the static TimeHelpers class which contains some
 * useful functions for working with dates
 */
...
public:
  ...
  static int get_simulation_time_birthday(const int &days_to_next_birthday,
                                          const int &age,
  ...
  static unsigned int days_in_month(const int year, const int month) {
    ...
    if (!is_leap_year(year)) { return 28; }
